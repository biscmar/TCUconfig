    ...
    $.ajax({
        ...
        success: function(r) {
            ...
            if (apiResponse.statuscode == 200) {
                ...
                var errMsgList = [
                    'Die Spieldaten konnten nicht geladen werden. Weitere Infos in der Browser-Konsole.'
                ];
...
function getRosterList(team) {
    ...
    if (
        $('#roster-' + team + '-starting-six').find(
            '.starting-six-option-selected'
        ).length == 1
    ) {
        switch (
            ...
            case team + '-starting-six-custom':
                rosterList.startingSix = $(
                    '#' + team + '-starting-six-tw'
                ).val();
                rosterList.startingSix +=
                    ',' + $('#' + team + '-starting-six-dl').val();
                rosterList.startingSix +=
                    ',' + $('#' + team + '-starting-six-dr').val();
                rosterList.startingSix +=
                    ',' + $('#' + team + '-starting-six-c').val();
                rosterList.startingSix +=
                    ',' + $('#' + team + '-starting-six-wl').val();
                rosterList.startingSix +=
                    ',' + $('#' + team + '-starting-six-wr').val();
                break;
...
function validateGameSettingsData() {
    ...
    if (
        ...
        errMsgList.push(
            'Heimteam und Auswärtsteam haben die selbe Trickot-Farbe'
        );
    }

    // Starting Six Option
    if (
        $('#roster-home-starting-six').find('.starting-six-option-selected')
            .length == 0
    ) {
    ...
    if (
        $('#roster-away-starting-six').find('.starting-six-option-selected')
            .length == 0
    ) {
